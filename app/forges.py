...
def fake_longnla(cnt=6):
    for i in range(cnt):
        loca = FONLoc(
            loc=i,
            longnla=str(fake.local_latlng(country_code='JP', coords_only=True))
        )
        db.session.add(loca)
    db.session.commit()
...
def fake_car(cnt=30):
    ...
    for i in range(cnt):
        car = CarAllinfo(item_id=1000000+i,
                         bmid=1000021 + random.randint(1, 2),
                         car_type=fake.vehicle_category(),
