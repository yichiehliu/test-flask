...
def queryforcar():
    ...
    # car_type = request.args.get('car_type', default='*', type=str)
    ...
    if (car_type == 'unspecified'):
        ...
        carlist = db.session.query(CarAllinfo).join(ReservedCarStatus).\
            filter(and_(CarAllinfo.car_type == car_type,  ReservedCarStatus.location.in_(loc_fon),
    ...
    for i in carlist:
        ...
        # try:
        print(i)
        did = int(time.time()) + int(qid) + cntd
        ...
        output.append([{'query_id': qid, 'location': locwlong, 'deal_id': did,
                        'brand_id': i.brand_id, 'bmid_id': i.bmid, 'car_type': i.car_type,
        ...
        #     output.update(
        #         {'query_id': qid, 'deal_id': did, 'brand_id': i.brand_id, 'bmid_id': i.bmid, 'car_type': i.car_type, 'model_id': i.model_id, 'price': str(i.price*rentalperiodcnt)})
...
def orderconfirmation():
    ...
    try:
        ...
        for i in alteramount:
            ...
            print(i.carAmount)
        output.update({
            "price": db.session.query(QueryRecord).filter(and_(QueryRecord.qid == qid, QueryRecord.did == did)).first().price,
            "message": True
        })
