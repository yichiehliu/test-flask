...
# @realtime_bp.route('/query/escooter', methods=['GET', 'POST'])
...
#     item_id_all = db.session.query(RealtimeCarDetails).filter(
#         RealtimeCarDetails.UsedorNot == False)
...
# @realtime_bp.route('/activate/escooter', methods=['GET', 'POST'])
...
#     item_id_all = db.session.query(RealtimeCarDetails).filter(
#         RealtimeCarDetails.item_id == item_id)
#     print(item_id_all)
#     for i in item_id_all:
#         i.UsedorNot = True
#         price = i.price
...
# @realtime_bp.route('/return/escooter', methods=['GET', 'POST'])
...
#     item_id = data['item_id']
#     et = date['end_time']
#     findcar = db.session.query(RealtimeCarDetails).filter(
#         RealtimeCarDetails.item_id == item_id)
#     findodr = db.session.query(RealtimeCarOrderRecord).filter(
#         RealtimeCarOrderRecord.oid == oid)
...
#         else:
#             # 另外算price，這裡先不定義price應該要怎麼算，先回傳false
#             output.update({'message': False, 'order_id': oid})
#         break
