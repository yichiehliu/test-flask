from app.models import db  # 不用套用實例?
...
class QueryRecord(db.Model):
    ...
    def __init__(self, id, qid, did, bmid, st, et, location, price):
        self.id = id
        self.qid = qid
        self.did = did
        self.bmid = bmid
...
class BookingRecord(db.Model):
    ...
    rid = db.Column(db.Integer, primary_key=True, db.ForeignKey(
        'QueryRecord.id', autoincrement=True)
...
class OrderRecord(db.Model):  # After activate the reservation
    ...
    item_id=db.Column(db.String, db.ForeignKey(
        'CarAllinfo.item_id'), nullable=False)
    st=db.Column(db.Integer, nullable=False)
    et=db.Column(db.Integer, nullable=False)
    location=db.Column(db.String, nullable=False)
    price=db.Column(db.Integer, nullable=False)
